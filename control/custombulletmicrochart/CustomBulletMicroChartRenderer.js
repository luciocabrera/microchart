(function() {
    ...
    if (bIsActualSet) {
      ...
      if (
        ...
        oRm.addStyle(
          'width',
          jQuery.sap.encodeHTML(
            Math.abs(oChartData.actualValuePct - oChartData.targetValuePct) +
              '%'
          )
        );
        oRm.addStyle(
          ...
          jQuery.sap.encodeHTML(
            1 +
              Math.min(oChartData.actualValuePct, oChartData.targetValuePct) +
              '%'
          )
        );
        oRm.writeStyles();
   ...
   */
  BulletMicroChartRenderer.renderThreshold = function(
    oRm,
    oControl,
    oThreshold,
    sSize
    ...
    if (
      sColor ===
      'sapSuiteBMCSemanticColor' + MobileLibrary.ValueColor.Error
    ) {
      oRm.write('<div');
      ...
      oRm.addStyle(
        jQuery.sap.encodeHTML(sDirection),
        jQuery.sap.encodeHTML(fValuePct + '%')
      );
      oRm.writeStyles();
      oRm.write('></div>');
    }
